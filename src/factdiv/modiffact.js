...
import { Select ,InputNumber,notification,Spin} from 'antd';
...
const openNotificationWithIcon = (type) => {
 ...
 class MFactdiv extends Component{
...
componentDidMount() {
   ...
   axios.get('/api/fact/get_fact/'+this.props.match.params.id,{ headers: { Authorization: `Bearer ${JSON.parse(localStorage.getItem("token"))}` }}).then(res=> {
      var data=res.data;
      var b;
      this.setState({
     ...
     this.setState({
       tab:[...this.state.tab,this.ta]
        ...
        b=(
          ...
          <td>{item.quantity}</td>
          <td>{item.tva}</td>
          <td><MinusCircleOutlined onClick={(e)=>{this.deletelement(e,item.id)}} id={i}/> &nbsp;&nbsp;<EditOutlined onClick={this.modifay} id={i}/></td>
          </tr>)
   
  this.setState({
    nb:i,
    afficherep:[...this.state.afficherep,b]
     })
   ...
   b;
    ...
    this.setState({
      ...
      axios.put('/api/detail/update_detail',{fact_id:this.props.match.params.id,client_id:this.state.client,voiture_id:this.state.voiture_id,societe_id:1,details:this.state.tab},{ headers: { Authorization: `Bearer ${JSON.parse(localStorage.getItem("token"))}` }})
      .then(response => { 
        this.setState({
          loading:false,
...
<div style={{textAlign:"center"}}>{this.state.loading?<LoadingPage/> :<></>}
    {!this.state.client  ?  <LoadingPage/> : 
         <div className="factdiv">
         <div className="modelclient">
         <p>Matricule : {this.state.matricule} </p>
             <p>Modele : {this.state.model} &nbsp; &nbsp;&nbsp;&nbsp;&nbsp; client : {this.state.client}</p>
         </div>
         <Table responsive>
      <thead>
        <tr>
